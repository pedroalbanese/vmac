...
func (h *Hash) SetNonce(n []byte) error {
	if len(n) >= h.cipher.BlockSize() {
		return fmt.Errorf("Nonce must be less than %d bytes", h.cipher.BlockSize())
	}
	h.nonce = n
	return nil
}

func New(cipher cipher.Block, key []byte, nonce []byte, size int) (*Hash, error) {
	if len(nonce) >= cipher.BlockSize() {
		return nil, errors.New(fmt.Sprintf("Nonce must be less than %d bytes", cipher.BlockSize()))
	}
	...
	h := &Hash{
		cipher:   cipher,
		blocklen: cipher.BlockSize() * 8,
		nonce:    nonce,
		size:     size,
		message:  bytes.NewBuffer(make([]byte, 0)),
	}
	return h, nil
}

func nonceSizeError(n int) error {
	return fmt.Errorf("Nonce must be less than %d bytes", n)
}
